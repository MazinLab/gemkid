...
HF_LL = layers.DrawingLayer((1, 1), "Hf_Bridge_Lincoln_Labs")
HF_CONTACT = layers.DrawingLayer((2, 0), "Hf_Contact")
HF_CONTACT_LIFTOFF = layers.DrawingLayer((3, 0), "Hf_Contact_Liftoff")
ASI = layers.DrawingLayer((4, 0), "AmorphousSilicon")
ASI_EP = layers.DrawingLayer((4, 1), "AmorphousSilicon_EtchProtect")
...
def drawing_to_sonnet(cell: gdstk.Cell):
    ...
    hf_ll_sim = gdstk.boolean(hf_ll, atanb, "not", 0.00001, *HF_LL_SONNET)
    hf_var = [
        p
        for p in cf.get_polygons()
    ...
    cell_sim.add(*atanb_sim, *hf_gp_sim, *hf_sim, *hf_gp_ll_sim, *hf_ll_sim, *hf_var)
    return cell_sim
...
def drawing_to_sonnet_3d(cell: gdstk.Cell):
    ...
    hf_ll_sim = gdstk.boolean(hf_ll, atanb, "not", 0.00001, *HF_LL_SONNET_3D)
    hf_var = [
        p
        for p in cf.get_polygons()
    ...
    cell_sim.add(*atanb_sim, *hf_gp_sim, *hf_sim, *hf_gp_ll_sim, *hf_ll_sim, *hf_var, *via_sim)
    return cell_sim
