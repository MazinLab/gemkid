...
def drawing_to_sonnet(cell: gdstk.Cell):
    ...
    hf_gp = [p for p in cf.get_polygons() if p.layer == HF_GP.gds_layer[0] and p.datatype == HF_GP.gds_layer[1]]
    ...
    cell_sim.add(*atanb_sim, *hf_gp_sim, *hf_sim, *hf_var)
    return cell_sim
...
def drawing_to_sonnet_3d(cell: gdstk.Cell):
    ...
    cell_sim.add(*atanb_sim, *hf_gp_sim, *hf_sim, *hf_var, *via_sim)
    return cell_sim
