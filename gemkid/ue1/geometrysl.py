...
from .layers import HF, HF_CONTACT
...
class UEFeedlineConfig(geometry.FeedlineConfig):
    ...
    b: float = 4
    c: float = 14
    feed_layer: tuple[int, int] | DrawingLayer = HF
    ground_layer: tuple[int, int] | DrawingLayer = HF
...
class BoxConfig(mecstyle.BoxConfig):
    ...
    feedline: UEFeedlineConfig = UEFeedlineConfig(feed_layer=HF, ground_layer=HF)
    coupler_width: float = 2.0
    coupler_gap: float = 3.5
    coupler_fill: bool = False
    coupler_layer: DrawingLayer = HF
    coupler_via: Optional[mecstyle.ViaWire] = (
        None  # mecstyle.ViaWire(2.0, HF, False, 2.0, 2.0, HF, 2.0, 2.0, HF_CONTACT)
    )
    box_width: float = 2.0
    box_gap: float = 1.0
    box_layer: DrawingLayer = HF
    width: float = 444
    height: float = 222
...
if __name__ == "__main__":
    ...
    def make_variant(variant):
        ...
        b = BoxConfig(InductorConfig(via_gap = 8, via_inset=0), CapacitorConfig(), UEFeedlineConfig())

        resonators = {
            4.100: [0.92478644, 0.94588675],
            4.150: [0.89059616, 0.91477959],
            4.200: [0.87025968, 0.88527906],
            4.250: [0.83854653, 0.85795688],
            4.300: [0.81257935, 0.83233653],
            4.350: [0.79458202, 0.80610407],
            4.400: [0.76086352, 0.78241044],
            6.100: [0.06177775, 0.23722607],
            6.200: [0.06446083, 0.21919269],
            6.300: [0.06639878, 0.2021521 ],
            6.400: [0.05986465, 0.1879494 ],
            7.500: [0.0420139 , 0.07098458],
            7.625: [0.03997454, 0.06232129],
            7.750: [0.03579341, 0.05477938],
            7.875: [0.0319396 , 0.04728761],
            8.000: [0.02895058, 0.03995088],
        }
        ...
        flstub.add(gdstk.rectangle((UEFeedlineConfig().width_half, 0), (444, 222), *HF))
        flstub.add(gdstk.rectangle((-UEFeedlineConfig().width_half, 0), (-444, 222), *HF))

        flstubmini.add(gdstk.rectangle((UEFeedlineConfig().width_half, 0), (444, 170), *HF))
        flstubmini.add(
            gdstk.rectangle((-UEFeedlineConfig().width_half, 0), (-444, 170), *HF)
        )
        ...
        for i in range(8):
            ...
            if variants[variant]:
                left.add(
                    gdstk.rectangle(
                        (b.focus_point[0] - variants[variant] / 2, b.focus_point[1] - (60 + 4) / 2),
                        (b.focus_point[0] + variants[variant] / 2, b.focus_point[1] + (60 + 4) / 2),
                        layer=3,
                        datatype=0,
                    )
        ...
        ec = gdstk.rectangle((-2700, 0), (2700, 950), *HF)
        f = UEFeedlineConfig()
        m = 3
        ...
        outline = gdstk.boolean(outline, gdstk.text("8 pH/sq v{:d}".format(variant), 250, (-2000, 250)), "not", layer=HF.gds_layer[0], datatype=HF.gds_layer[1])
        ...
        tlm = [
            gdstk.rectangle(
                (-2700 + 150 + 150 + 200 + 8, 2700 // 2 - 4),
                (-2700 + 150 + 150 + 200 + 8 + sum(LS) + len(LS) * 32 + 32, 2700 // 2 + 4),
            )
        ]
        for i in range(0, len(LS) + 1):
            s = sum(LS[:i]) + 16 + 4 + i * 32
            if variant % 2 == 1 and i != len(LS):
                    top.add(
                        gdstk.rectangle(
                            (-2700 + 150 + 150 + 200 + 8 + sum(LS[:i]) + i * 32 + 32, 2700 // 2 - 8),
                            (-2700 + 150 + 150 + 200 + 8 + sum(LS[:i]) + i * 32 + 32 + LS[i], 2700 // 2 + 8),
                            layer=3,
                            datatype=0
                        )
                    )
        ...
        rect_right = gdstk.boolean(rect_right, milo, "or", layer=HF.gds_layer[0], datatype=HF.gds_layer[1])
        rect_left = gdstk.boolean(rect_left, rect_left, "or", layer=HF.gds_layer[0], datatype=HF.gds_layer[1])
            
        top.add(*gdstk.boolean(tlm, tlm, "or", layer=HF.gds_layer[0], datatype=HF.gds_layer[1]))
    ...
    for cord in [(-21000, -21000), (-21000, 21000), (21000, -21000), (21000, 21000), (0, 0)]:
        ...
        top.add(gdstk.cross(cord, 800, 50, layer=3, datatype=0))
        top.add(
            *gdstk.boolean(
                gdstk.rectangle(
                    (cord[0] - 600, cord[1] - 600),
                    (cord[0] + 600, cord[1] + 600),
                ),
                gdstk.rectangle(
                    (cord[0] - 500, cord[1] - 500),
                    (cord[0] + 500, cord[1] + 500),
                ),
                "not",
                layer = 3,
