...
class InductorConfig(mecstyle.InductorDoubledConfig):
    ...
    def draw(self, port_offset=0, variation_layer=None, cellcache=...):
        ...
        asi = [
            gdstk.rectangle(
                (
                    self.wiring_width * 2 + self.wiring_gap * 2 + self.wiring_extra + ASI_PADDING,
                    self.wiring_gap
                    + self.wiring_extra_height
                    + (self.wiring_width - self.leg_width) / 2
                    - via_over
            ...
            gdstk.rectangle(
                (
                    self.wiring_width * 2 + self.wiring_gap * 2 + self.wiring_extra,
                    self.wiring_gap
                    + self.wiring_extra_height
                    + (self.wiring_width - self.leg_width) / 2
                    - via_over,
                ...
                (
                    self.wiring_width * 2 + self.wiring_gap * 2 + self.wiring_extra + self.leg_length,
                    self.dimensions[1] + via_over,
                ),
                *ASI_EP,
            ),
...
class CapacitorConfig(mecstyle.CapacitorConfig):
    legs: int = 42
    leg_gap: float = 1
    leg_length: tuple[float, float] = (222 - 32 - 2 - 8 - 3.5, 140)
    leg_width: float = 2
...
if __name__ == "__main__":
    ...
    def make_variant(variant):
        ...
        flstub.add(gdstk.rectangle((UEFeedlineConfig().width_half, 0), (222, 222), *ATA_NB))
        flstub.add(gdstk.rectangle((-UEFeedlineConfig().width_half, 0), (-222, 222), *ATA_NB))

        flstubmini.add(gdstk.rectangle((UEFeedlineConfig().width_half, 0), (222, 170), *ATA_NB))
        flstubmini.add(gdstk.rectangle((-UEFeedlineConfig().width_half, 0), (-222, 170), *ATA_NB))
        flstub.name = "flstubv{:d}".format(variant)
        flstubmini.name = "flstubmini{:d}".format(variant)

        rect_right = gdstk.rectangle((222, 0), (2700, 3500))
        rect_right = gdstk.boolean(rect_right, gdstk.rectangle((2700 - 1422.4, 0), (2700, 1524)), "not")
        rect_left = gdstk.rectangle((-222, 0), (-2700, 3500))
        ...
        outline = gdstk.boolean(
            outline,
            gdstk.text("8 pH/sq ATANB v{:d}".format(variant), 250, (-2500, 250)),
            "not",
    ...
    lib.write_gds("ue2tm4style8ph.gds")
